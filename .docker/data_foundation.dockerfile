...
FROM python:3.12-slim-bullseye AS dbt_compiler
    ...
    RUN --mount=type=secret,id=destination__user \
        ...
        --mount=type=secret,id=destination__host \
        --mount=type=secret,id=destination__role \
        --mount=type=secret,id=destination__password \
        --mount=type=secret,id=destination__warehouse \
        export DESTINATION__USER=$(cat /run/secrets/destination__user) \
        && export DESTINATION__DATABASE=$(cat /run/secrets/destination__database) \
        && export DESTINATION__HOST=$(cat /run/secrets/destination__host) \
        ...
        && pip install "dbt-core==1.10.11" "dbt-snowflake" \
